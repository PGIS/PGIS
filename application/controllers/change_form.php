...
class Change_form extends CI_Controller {
    ...
    function viewcal($data=0){
         if ($this->session->userdata('user_role') == 'applicant') {
            $this->load->view('application/chang_pwd',$data);
        } elseif ($this->session->userdata('user_role') == 'Admision staff') {
            $this->load->view('Admision/chang_pwd',$data);
        } elseif ($this->session->userdata('user_role') == 'Finance staff') {
            $this->load->view('finance/chang_pwd',$data);
        } elseif ($this->session->userdata('user_role') == 'administrator') {
            $this->load->view('admin/chang_pwd',$data);
        } elseif ($this->session->userdata('user_role') == 'Student') {
            $this->load->view('registration/chang_pwd',$data);
        } elseif ($this->session->userdata('user_role') === 'Supervisor') {
            $this->load->view('academic/change_pwd',$data);
        } elseif ($this->session->userdata('user_role') === 'Teaching staff') {
            $this->load->view('academic/change_pwds',$data);
        } elseif ($this->session->userdata('user_role') === 'alumni') {
