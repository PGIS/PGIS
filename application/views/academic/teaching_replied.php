            ...
            <table class="table table-bordered table-striped">
                <?php if(isset($replied)){
                 ...
                 foreach ($replied->result() as $rec){
                     echo '<tr><td>'.$rec->registrationID.'</td><td>'.$rec->surname.'</td><td>'.$rec->other_name.'</td><td>'.substr($rec->document,39).'</td><td>'.$rec->submission_date.'</td><td>'.anchor('teaching/resert/'.$rec->id,'<span class="badge">Resert</span>').'</td></tr>';
                 }
                }  else {
