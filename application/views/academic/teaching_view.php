            ...
            <table class="table table-striped" id="mytablet2">
                ...
                <?php if(isset($record)){
                 foreach ($record->result() as $rec){
                     echo '<tr><td>'.$rec->registrationID.'</td><td>'.$rec->surname.' '.$rec->other_name.'</td><td>'.substr($rec->document,39).'</td><td>'.$rec->submission_date.'</td><td>' .' '.anchor('teaching/download/'.$rec->id,'Download',array('class'=>'glyphicon glyphicon-download-alt')).'</td><td>'.anchor('teaching/view/'.$rec->id,'<span class="badge">View & comment</span>').'</td></tr>';
                 }
                }  
                ?>
                </tbody>
