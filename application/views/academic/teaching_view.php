            ...
            <table class="table table-bordered table-striped">
                ...
                <?php if(isset($record)){
                 ...
                 foreach ($record->result() as $rec){
                     echo '<tr><td>'.$rec->registrationID.'</td><td>'.$rec->surname.'</td><td>'.$rec->other_name.'</td><td>'.substr($rec->document,39).'</td><td>'.$rec->submission_date.'</td><td><span class="glyphicon glyphicon-download-alt"></span>' .' '.anchor('teaching/download/'.$rec->registrationID,'Download').'</td><td>'.anchor('teaching/view/'.$rec->registrationID,'<span class="badge">View</span>').'</td></tr>';
                 }
                }  else {
