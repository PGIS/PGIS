            ...
            <div class=" tab-content" style="display:block;">
                <div class="in tab-pane <?php if(isset($active)){echo 'active';}?>" id="internal">
                    ...
                    <table class="table table-striped" id="internaz">
                        <thead><tr><th>REGISTRATION ID</th><th>FULL NAME</th><th>DOCUMENT</th><th>EXAMINER</th><th>ACTION</th><th>CHOOSE</th><th>EXTRA</th></tr></thead>
                        <tbody>
                            ...
                            foreach ($internal->result() as $row){
                                ...
                                echo '<tr><td>'.$row->registrationID.'</td><td>'.$row->surname.' '.$row->other_name.'</td><td>'.substr($row->int_document, 39).'</td><td>'.$row->internal_examiner.'</td>'
                                   . '<td>'.anchor('college_Coordinator/internaldownload/'.$row->int_id,'<button class="btn btn-success btn-xs"><span class="glyphicon glyphicon-download-alt"></span> Downladfile</button>').'</td>'
                                        . '<td><div class="btn-group btn-group-xs"><button class="btn btn-info btn-xs" data-toggle="dropdown">Final grade <span class="caret"></span></button>'
                                        . '<ul class="dropdown-menu" role="menu">'
                                        . '<li onclick="StudentGraduate(\''.$row->int_id.'\')" data-target=".graduate" data-toggle="modal"><a href="#">Graduate</a></li>'
                                        . '<li onclick="StudentNotGraduate(\''.$row->int_id.'\')" data-target=".graduate" data-toggle="modal"><a href="#">No graduate</a></li>'
                                        . '</ul>'
                                        . '</div>'
                                        . '</td><td><button class="btn btn-warning btn-xs" onclick="viewInternalFd(\''.$row->int_id.'\')" data-target=".viewcomment" data-toggle="modal">view comments</button></td></tr>';
                            }  else {
                               echo '<tr><td>'.$row->registrationID.'</td><td>'.$row->surname.' '.$row->other_name.'</td><td>'.substr($row->int_document, 39).'</td><td>'.$row->internal_examiner.'</td>'
                    ...
                    <div class="modal fade graduate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-md">
                      <div class="modal-content">
                        <div class="modal-header">
                          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                          <h6 class="modal-title" id="myModalLabel"><label class="pantop">Final grade reached</label></h6>
                        </div>
                        <div class="modal-body">
                            <div class="grads"></div>
                        </div>
                            ...
                            foreach ($external->result() as $rows){
                                ...
                                echo '<tr><td>'.$rows->registrationID.'</td><td>'.$rows->surname.' '.$rows->other_name.'</td><td>'.substr($rows->ext_document, 39).'</td><td>'.$rows->external_examiner.'</td>'
                                   . '<td>'.anchor('college_Coordinator/externaldownload/'.$rows->feed_id,'<button class="btn btn-success btn-xs"><span class="glyphicon glyphicon-download-alt"></span> Downlad file</button>').'</td>'
                                        ...
                                        . '<li><a href="#">No graduate</a></li>'
                                        . '</ul>'
                                        . '</div></td><td><button class="btn btn-warning btn-xs" onclick="viewExternalComment(\''.$rows->feed_id.'\')" data-target=".viewcommentext" data-toggle="modal">view comments</button></td></tr>';
                             }  else {
                               echo '<tr><td>'.$rows->registrationID.'</td><td>'.$rows->surname.' '.$rows->other_name.'</td><td>'.substr($rows->ext_document, 39).'</td><td>'.$rows->external_examiner.'</td>'
                                   . '<td><p class="alert alert-warning"> No file attached</p></td>'
                                       . '<div class="btn-group btn-group-xs"><button class="btn btn-info btn-xs" data-toggle="dropdown">Final grade <span class="caret"></span></button>'
                                        . '<ul class="dropdown-menu" role="menu">'
                                        . '<li><a href="#">Graduate</a></li>'
                                        . '<li><a href="#">No graduate</a></li>'
                                        . '</ul>'
                                        . '</div></td><td><button class="btn btn-info btn-xs" onclick="viewExternalComment(\''.$rows->feed_id.'\')" data-target=".viewcommentext" data-toggle="modal">view comments</button></td></tr>'; 
                            }
                            }
                            }?>
                        </tbody>
