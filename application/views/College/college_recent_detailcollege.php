<?php 
if($this->session->userdata('user_role')==='college coordinator'){
 include_once 'Headerlogin.php';   
}elseif ($this->session->userdata('user_role')==='college principle') {
 include_once 'Headerlogincollege.php';  
}
?>
<div id="page-wrapper">
    <div class="span12">
                 ...
                 if($result->num_rows()>0){
                  ...
                  foreach ($result->result() as $ver){
                         echo '<tbody>'
                            ...
                            . '<td>'.$ver->pr_date.'</td>'
                            . '<td>'.$ver->type.'</td>'
                            . '<td><button class="btn btn-success btn-xs" onclick="studentviewCollege(\''.$ver->ver_id.'\')" data-target="#student" data-toggle="modal"><span class="glyphicon glyphicon-share">'
                            . '</span>view</button></td><td>'.anchor('college/downloadpdf/'.$ver->ver_id,'<button class="btn btn-success btn-xs"><span class="glyphicon glyphicon-download-alt">Download</span></button>').'</td>'
                          . '</tr></tbody>';
                     }
                 }
                 }  else {
...
<script>
 ...
 function studentviewCollege(id){
